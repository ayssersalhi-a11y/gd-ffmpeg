 ...
 *        "الكودك جاهز" و"أول إطار مفكوك" — ثابتة بغض النظر عن سرعة الشبكة
 *        (لوحظت مع اتصال بطيء وسريع على حدٍّ سواء)، مما يرجّح أنها تكلفة
 *        إنشاء جلسة MediaCodec الحقيقية مع سائق الجهاز، لا انتظار بيانات.
 *        الآن تُفتَح جلسة h264_mediacodec وهمية (1280×720، بلا بيانات فعلية)
 *        في خيط خلفي منفصل تمامًا عند _ready() — مرة واحدة فقط لعمر التطبيق
 *        (وليس لكل فيديو) — أثناء الوقت "الميت" (تصفّح المستخدم) بدل حدوثها
 *        على المسار الحرج لأول تشغيل فعلي. فشلها آمن تمامًا ولا يؤثر على
 *        التشغيل الحقيقي إطلاقًا (سياق منفصل يُغلَق فورًا). لا يقين كامل أن
 *        هذا يلتقط كامل الفجوة (قد تحتاج فك إطار حقيقي أيضًا)، لكنه خالٍ من
 *        أي مخاطرة كخطوة أولى.
 *
 * ─── الجديد في v7.4 ──────────────────────────────────────────────────────────
 *
...
private:
    ...
    // ── [DECODER-WARMUP v7.5] تسخين مُفكِّك MediaCodec مسبقًا ─────────────────
    // دالة ساكنة عمدًا (لا تعتمد على this) — تعمل في خيط منفصل تمامًا
    // (detach) دون أي ارتباط بعمر كائن FFmpegPlayer نفسه، فلا خطر من مؤشر
    // معلَّق (Dangling this) إن أُغلق المشهد قبل انتهاء التسخين. تُنفَّذ مرة
    // واحدة فقط لكل عمر التطبيق (وليس لكل فيديو) عبر warmup_done الساكن.
